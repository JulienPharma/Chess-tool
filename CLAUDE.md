...
- **Backend : Supabase** — projet dédié **`sponqvqqfcedkkfvasfw`** (« Chess-tool », région **eu-west-2**). URL: `https://sponqvqqfcedkkfvasfw.supabase.co`. La clé publishable est publique par design (voir `.env.local.example`).
  ...
  **Supabase Auth** en magic link. Attention : le magic link laisse n'importe qui devenir `authenticated` — les policies RLS filtrent donc sur `owner_id = auth.uid()`, et il faut désactiver les inscriptions dans les réglages Auth.
